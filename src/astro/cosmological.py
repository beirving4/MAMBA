...
class Cosmology: 
    ...
    matter: float
    darkEng: float 
    baryon: float = field(default=astropy.cosmology.Planck18.Ob0)  
    curve: float = field(default=None)
    ...
    def __post_init__(self) -> None:
        ...
        if self._cosmo is None:  
            astropy_cosmo = astropy.cosmology.FlatLambdaCDM(
                self.hubble, self.matter, Ob0=self.baryon
            )
            self._cosmo = cosmology.fromAstropy(
                astropy_cosmo, sigma8=0.8, ns=0.97, cosmo_name="customCosmo"
            )

        # Cosmological Curvature Density
        self.curve = self._cosmo.Ok0 if (self.curve is None) else self.curve

    def __repr__(self) -> str:
        return (
            f"Cosmology(h0 = {self.h0:.3f}, " 
            f"Om0 = {self.matter:.3f}, Ode0 = {self.darkEng:.3f})"
        )
    ...
    def lookback_time_to_redshift(self, lookback_time : float) -> float:
        return self._cosmo.lookbackTime(
            (lookback_time * u.Myr).to_value("Gyr"), inverse=True
        ) 
...
def getCosmology(
    ...
    simulation : Optional[arborType]=None,
    ...
    if (model is not None):
        if (model.lower() not in cosmology.cosmologies.keys()):
            raise KeyError("Input is not a defined cosmology")
        elif (model.lower() == "powerlaw"):
            raise Exception(
                "Power Law Cosmologies Haven't Been Implemented Yet!"
            )  
        else: 
            cosmo = Cosmology.fromDefinedCosmologies(model.lower())
